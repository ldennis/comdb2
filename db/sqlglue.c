...
static inline char *create_systime_trigger_query(struct db *db, int is_update)
{
    ...
    ret = strdup(strbuf_buf(sql));

    strbuf_free(sql);
    return ret;
}

static inline char *create_bustime_trigger_query(struct db *db)
{
    struct schema *schema;
    strbuf *sql;
    int field;
...
static int create_sqlmaster_record(struct db *db, void *tran)
{
    ...
    if (db->systime_sql) free(db->systime_sql);
    db->systime_sql = NULL;
    if (db->periods[PERIOD_SYSTEM].enable) {
        strbuf_clear(sql);
        ...
        strbuf_append(sql, "\"");
        strbuf_append(sql, ";");
        db->systime_sql = strdup(strbuf_buf(sql));
    }
    if (db->systime_upd_sql) free(db->systime_upd_sql);
    db->systime_upd_sql = create_systime_trigger_query(db, 1 /* is_update */);
    if (db->systime_del_sql) free(db->systime_del_sql);
    db->systime_del_sql = create_systime_trigger_query(db, 0);

    if (db->bustime_sql) free(db->bustime_sql);
    db->bustime_sql= create_bustime_trigger_query(db);
...
enum MASTER_TABLE_ENTRY {
    TABLE_ENTRIES = 0,
    SYSTIME_VIEW = 1,
    SYSTIME_UPD_TRIGGER = 2,
    SYSTIME_DEL_TRIGGER = 3,
    BUSTIME_TRIGGER = 4
};
static void *create_master_table(int eidx, const char *csc2_schema, int tblnum,
                                 int ixnum, int *sz, int type)
{
    ...
    if (type == SYSTIME_VIEW) {
        snprintf(name, sizeof(name), "%s_systime", dbname);
        sql = db->systime_sql;
        etype = "view";
    } else if (type == SYSTIME_UPD_TRIGGER) {
        snprintf(name, sizeof(name), "%s_systime_upd", dbname);
        sql = db->systime_upd_sql;
        etype = "trigger";
    } else if (type == SYSTIME_DEL_TRIGGER) {
        snprintf(name, sizeof(name), "%s_systime_del", dbname);
        sql = db->systime_del_sql;
        etype = "trigger";
    } else if (type == BUSTIME_TRIGGER) {
        snprintf(name, sizeof(name), "%s_bustime", dbname);
        sql = db->bustime_sql;
        etype = "trigger";
    } else if (ixnum == -1) {
        strcpy(name, dbname);
...
void create_master_tables(void)
{
    ...
    for (tblnum = 0; tblnum < thedb->num_dbs; tblnum++) {
        ...
        if (thedb->dbs[tblnum]->systime_sql)
            ++local_nentries;
        if (thedb->dbs[tblnum]->systime_upd_sql)
            ++local_nentries;
        if (thedb->dbs[tblnum]->systime_del_sql)
            ++local_nentries;
        if (thedb->dbs[tblnum]->bustime_sql)
            ++local_nentries;
    }
    ...
    for (tblnum = 0; tblnum < thedb->num_dbs; tblnum++) {
        ...
        gbl_master_table_entries[eidx] =
            create_master_table(eidx, thedb->dbs[tblnum]->csc2_schema, tblnum,
                                -1, &gbl_master_entry_sizes[eidx], TABLE_ENTRIES);
        eidx++;
#if 0
      printf
        ...
        for (ixnum = 0; ixnum < nix; ixnum++) {
            ...
            if (thedb->dbs[tblnum]->ixsql[ixnum] != NULL) {
                ...
                gbl_master_table_entries[eidx] =
                    create_master_table(eidx, NULL, tblnum, ixnum,
                                        &gbl_master_entry_sizes[eidx], TABLE_ENTRIES);
#if 0
            printf
    ...
    for (tblnum = 0; tblnum < thedb->num_dbs; tblnum++) {
        if (thedb->dbs[tblnum]->systime_sql) {
            gbl_master_table_entries[eidx] = create_master_table(
                eidx, NULL, tblnum, -1, &gbl_master_entry_sizes[eidx],
                SYSTIME_VIEW);
            eidx++;
        }
        if (thedb->dbs[tblnum]->systime_upd_sql) {
...
int is_comdb2_temporal_table(const char *tbl, char **start, char **end, int pd)
{
    ...
    if (db->periods[pd].enable) {
        if (start)
            *start = schema->member[db->periods[pd].start].name;
        if (end)
            *end  = schema->member[db->periods[pd].end].name;
        return 1;
    }
    return 0;
}
                             ...
                             int isBus, void *pCaller)
{
    ...
    if (thd) {
        if (clnt->pTemporalParser && clnt->pTemporalParser != pCaller)
            return;
...
int comdb2_temporal_systime_check(Vdbe *v, const char *dbname, Mem *pMem)
{
    ...
    if (dt.dttz_sec > pMem->du.dt.dttz_sec ||
        ...
        sqlite3VdbeError(v, "Invalid parameter: table '%s' was not valid until "
                         "'%s', given '%s'", dbname, timestr, timestr2);
        return SQLITE_ERROR;
	}
	return SQLITE_OK;
}
...
int temporal_business_time_check(struct db *db, uint8_t *od_dta)
{
    ...
    if (rc >= 0) {
        if(stype_is_null(od_dta + fend->offset))
            return 0;
        return 1;
    }
    return 0;
