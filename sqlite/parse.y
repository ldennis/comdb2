...
// COMDB2 KEYWORDS  
//
  ADD AGGREGATE ALIAS AUTHENTICATION BLOBFIELD BULKIMPORT CHECK COMMITSLEEP
  CONSUMER CONVERTSLEEP COVERAGE CRLE DATA DATABLOB DATACOPY DBPAD DEFERRABLE
  DISABLE DRYRUN ENABLE FOR FUNCTION GENID48 GET GRANT IPU ISC KW LUA LZ4 NONE
  ODH OFF OP OPTIONS PAGEORDER PARTITION PASSWORD PAUSE PERIOD PROCEDURE PUT
  REBUILD READ READONLY REC RESERVED RESUME RETENTION REVOKE RLE ROWLOCKS SCALAR
  SCHEMACHANGE SKIPSCAN START SUMMARIZE THREADS THRESHOLD TIME TRUNCATE TUNABLE
...
scaction(A) ::= DETACH. { A = SC_ACTION_DETACH; }
