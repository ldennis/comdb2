...
void sqlite3BeginTrigger(
  ...
  assert( pName2!=0 );
  assert( op==TK_INSERT || op==TK_UPDATE || op==TK_DELETE || op==TK_BUSINESS_TIME );
  assert( op>0 && op<0xff );
  if( isTemp ){
    ...
    switch( pStep->op ){
      ...
      case TK_DELETE: {
        sqlite3DeleteFrom(pParse, 
          targetSrcList(pParse, pStep),
          sqlite3ExprDup(db, pStep->pWhere, 0), 0, 0, 0, NULL
        );
        break;
      }
...
void sqlite3CodeRowTrigger(
  ...
  assert( op==TK_BUSINESS_TIME || (op==TK_UPDATE)==(pChanges!=0) );
